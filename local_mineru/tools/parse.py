...
from dify_plugin.invocations.file import File, UploadFileResponse
...
class MineruTool(Tool):
    ...
    def validate_token(self) -> None:
        ...
        if credentials.server_type=="local":
            ...
            response = get(url, headers=self._get_headers(credentials), timeout=10, verify=False)
            ...
            response = post(url, headers=self._get_headers(credentials), timeout=10, verify=False)
            if response.status_code!= 200:
    ...
    def _parser_file_local(self, credentials: Credentials, tool_parameters: Dict[str, Any]):
        ...
        response = post(task_url, headers=headers, params=params, files=file_data, verify=False)
        if response.status_code != 200:
    ...
    def _parser_file_remote(self, credentials: Credentials, tool_parameters: Dict[str, Any]):
        ...
        response = post(task_url, headers=header, json=data, verify=False)
        ...
        if result["code"] == 0:
            ...
            res_upload = put(urls[0], data=file.blob, verify=False)
